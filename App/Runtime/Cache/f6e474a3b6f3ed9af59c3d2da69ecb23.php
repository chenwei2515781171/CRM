...
<div class="container">
	...
	<li <?php if($active == 1): ?>class="active"<?php endif; ?>><a href="<?php echo U('setting/defaultInfo');?>"><?php echo L('BASIC_SYSTEM_SETTING');?></a></li>
	<!--li><a href="<?php echo U('setting/smtp');?>"><?php echo L('SMTP_SETTING');?></a></li-->
	<li <?php if($active == 2): ?>class="active"<?php endif; ?>><a href="<?php echo U('setting/fields');?>"><?php echo L('CUSTOMIZING_FIELDS_SETTING');?></a></li>		
	<li <?php if($active == 3): ?>class="active"<?php endif; ?>><a href="<?php echo U('navigation/setting');?>"><?php echo L('SYSTEM_NAVIGATION_SETTING');?></a></li>
	<!--li><a href="<?php echo U('setting/weixin');?>"><?php echo L('WHCHAT_SETTING');?></a></li-->
	<!--li <?php if($active == 4): ?>class="active"<?php endif; ?>><a href="<?php echo U('setting/subCompany');?>"><?php echo L('SUBCOMPANY');?></a></li-->
</ul>
		<div class="row">
			<form class="form-horizontal" enctype="multipart/form-data" action="<?php echo U('setting/defaultInfo');?>" method="post">
				<table class="span12 table">
					...
					<tbody>
						...
						<tr>
							<td class="tdleft"><?php echo L('BONUS_POOL');?></td>
							<td>
								<input class="input-small" type="text" id="bonus_pool" name="bonus_pool" value="<?php echo ($bonus_pool); ?>" size="5"/>%
							</td>
						</tr>
						<tr>
							<th colspan="2"><?php echo L('SEND_MESSEGE');?></th>
						</tr>
						<tr>
							<td class="tdleft"><?php echo L('SEND_CHECK');?></td>
							<td>
								<?php if(is_array($user_list)): $i = 0; $__LIST__ = $user_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><input style="margin-left:20px;" <?php if(in_array($vo[user_id],$check_send)): ?>checked="checked"<?php endif; ?> class="input-small" type="checkbox" name="check_send[]" value="<?php echo ($vo["user_id"]); ?>"/><?php echo ($vo["name"]); endforeach; endif; else: echo "" ;endif; ?>
							</td>
						</tr>
						
						<tr>
							<td class="tdleft"><?php echo L('CREATE_SALE_USER');?></td>
							<td>
								<?php if(is_array($user_list)): $i = 0; $__LIST__ = $user_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><input style="margin-left:20px;" <?php if(in_array($vo[user_id],$create_sale_user)): ?>checked="checked"<?php endif; ?> class="input-small" type="checkbox" name="create_sale_user[]" value="<?php echo ($vo["user_id"]); ?>"/><?php echo ($vo["name"]); endforeach; endif; else: echo "" ;endif; ?>
							</td>
						</tr>
						<tr>
							<td class="tdleft"><?php echo L('SEND_OUTORDER');?></td>
							<td>
								<?php if(is_array($user_list)): $i = 0; $__LIST__ = $user_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><input style="margin-left:20px;" <?php if(in_array($vo[user_id],$outorder_send)): ?>checked="checked"<?php endif; ?> class="input-small" type="checkbox" name="outorder_send[]" value="<?php echo ($vo["user_id"]); ?>"/><?php echo ($vo["name"]); endforeach; endif; else: echo "" ;endif; ?>
							</td>
						</tr>
						<tr>
							<td>&nbsp;</td>  
