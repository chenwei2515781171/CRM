						...
						<form class="form-inline" id="searchForm" onsubmit="return checkSearchForm();" action="" method="get">
							...
							<li class="pull-left" style="margin-top:3px;">
								供货公司&nbsp; <select style="width:auto" name="supplier_name" id="supplier_name" >
									<option class="all" value="all"><?php echo L('ALL');?></option>
									<?php if(is_array($supplierlist)): $i = 0; $__LIST__ = $supplierlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>"><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
								</select>&nbsp;&nbsp;
							</li>
                           <li class="pull-left">
			...
			<table class="table table-hover ">
				...
				<tbody>
                        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(is_array($vo["data"])): $k = 0; $__LIST__ = $vo["data"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sub): $mod = ($k % 2 );++$k;?><tr>
								...
								<td><?php echo ($sub["supplier_name"]); ?></td>
								<td><?php echo ($sub["warehouse_name"]); ?></td>
								<td><?php echo ($sub["create_user_name"]); ?></td>
								<td><?php echo ($sub["check_user_name"]); ?></td>
								<td><?php echo (date("Y-m-d",$sub['create_time'])); ?></td>								
								</tr><?php endforeach; endif; else: echo "" ;endif; ?>
							<tr>
								<td></td>
								<td colspan="3">入库金额总计：<?php echo ($vo["total_all"]); ?>元</td>
								<td colspan="3">入库量总计：<?php echo ($vo["num_all"]); ?> </td>
...
$("#inexcelExport").click(function(){
	if(confirm("你确定要导出出库数据吗？")){
		...
		var url = "<?php echo U('stock/in');?>&product="+product+"&warehouse="+warehouse+"&supplier_name="+supplier_name+"&start_time="+start_time+"&end_time="+end_time+"&act=inexcel";
