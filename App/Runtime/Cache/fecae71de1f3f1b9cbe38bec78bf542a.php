...
<div class="container">
	...
	<a href="<?php echo U('leads/index','by=public');?>" <?php if($_GET['by']== 'public'): ?>class="active"<?php endif; ?>><?php echo L('LEADS_POOL');?></a>  &nbsp;  &nbsp; 
				...
				<li class="pull-left">
					<form class="form-inline" id="searchForm"  action="index.php" method="get">
						<ul class="nav pull-left">
							...
							<button type="submit" id="dosearch" class="btn"> <img src="__PUBLIC__/img/search.png"/><?php echo L('SEARCH');?></button> &nbsp;</li>
							<li class="pull-left">
				...
				<?php else: ?>
					<thead>						
						<tr id="childNodes_num">
							...
							<?php if($_GET['by']!= 'public' && $_GET['by']!= 'deleted'): ?><th><?php echo L('DEADLINE');?></th><?php endif; ?>
					...
					<tfoot>
						<tr>
							<?php if($_GET['by']== 'transformed'): ?><td id="td_colspan"><?php echo ($page); ?></td>
							<?php elseif($_GET['by']== 'deleted'): ?>
								<td id="td_colspan"><?php echo ($page); ?></td>
							<?php else: ?>
								<td id="td_colspan"><?php echo ($page); ?></td><?php endif; ?>							
						</tr>
					</tfoot>
					<tbody>
						<?php if(is_array($leadslist)): $i = 0; $__LIST__ = $leadslist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								...
								<?php if($_GET['by']!= 'public' && $_GET['by']!= 'deleted'): ?><td><?php if($vo['days'] <= 7): ?><font color="red"><?php echo ($vo["days"]); echo L('DAYS');?></font><?php else: ?><font color="blue"><?php echo ($vo["days"]); echo L('DAYS');?></font><?php endif; ?></td><?php endif; ?>
								<?php if($_GET['by']!= 'transformed'): ?><td>
									<a href="<?php echo U('leads/view', 'id=' . $vo['leads_id']);?>"><?php echo L('VIEW');?></a>&nbsp;<?php endif; ?>
								<?php if(($_GET['by']!= 'transformed') and ($_GET['by']!= 'deleted')): ?><a  target="_blank" href="<?php echo U('customer/add','leads_id='.$vo['leads_id']);?>"><?php echo L('CONVERT');?></a>&nbsp;<?php endif; ?>
