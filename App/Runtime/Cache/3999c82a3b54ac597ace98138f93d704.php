...
<div class="container">
	...
	<a href="<?php echo U('finance/zensong');?>">赠送</a> &nbsp;  &nbsp; &nbsp;  &nbsp; 
	<div>
	</div>
	<div class="row">
		<div class="span12">
						...
						<ul class="nav pull-left" style="margin-top:20px">
							...
							<li class="pull-left">
								选择员工&nbsp; <select style="width:auto" name="role" id="role" onchange="changeCondition()">
									<option class="all" value="all"><?php echo L('ALL');?></option>
									<?php if(is_array($salesuserlist)): $i = 0; $__LIST__ = $salesuserlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["role_id"]); ?>"><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
								</select>&nbsp;&nbsp;
							</li>
							<li class="pull-left">
								日期&nbsp; <?php echo L('FROM');?> <input id="start_time" name="start_time" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" class="Wdate span2" type="text"> <?php echo L('TO');?> <input id="end_time" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" name="end_time" class="Wdate span2" type="text">&nbsp;&nbsp;
							</li>
							<li class="pull-left">
								<input type="hidden" name="act" id="act" value="search"/>
								<input type="hidden" name="zsact" id="zsact" value="search"/>
								<input name="m" value="finance" type="hidden">
								<input name="a" value="zensong" type="hidden">
				...
				<table class="table table-hover">
					<thead>
						<tr>
							...
							<th><a href="javascript:void(0);" id="zengsongexcel" class="link"> <i class="icon-download"> </i> 导出表格</a></th>
						</tr>
						<tr>
							<th>客户</th>
							<th>销售单号</th>
					...
					<tbody>
                        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(is_array($vo["data"])): $k = 0; $__LIST__ = $vo["data"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sub): $mod = ($k % 2 );++$k;?><tr>
								...
								<td><a href="<?php echo U('sales/view','id='.$sub['sales_id']);?>"><?php echo ($sub["sn_code"]); ?></a></td>
								<td><?php echo ($sub["sales_price"]); ?></td>
								<td><?php echo ($sub["sale_user_name"]); ?></td>
								<td><?php echo (date("Y-m-d",$sub['create_time'])); ?></td>	
								<td><a class="view_product" rel="<?php echo ($sub["sales_id"]); ?>" href="javascript:void(0);">查看</a></td>
...
$("#zengsongexcel").click(function(){
	if(confirm("你确定要导出赠送数据吗？")){
		$("#zsact").val('zsexcel');	
		$("#searchForm").submit();
	}
})
