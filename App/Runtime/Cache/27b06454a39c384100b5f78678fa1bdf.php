			...
			<table class="table table-hover table-striped table_thead_fixed"> 
				<?php if(!empty($list)): ?><thead>
					<tr>
						...
						<th>涉及单据</th>
						<th>类型</th>
						<?php if(C('ismobile') != 1): ?><th><?php echo L('AMOUNT PAYABLE');?></th>
							<!--th><?php echo L('PAYABLES TIME');?></th-->
							...
							<?php elseif(C('ismobile') != 1): ?>
								...
								<th><?php echo L('CREATE_TIME');?></th><?php endif; endif; ?>
				...
				<tbody>
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
							...
							<td><?php if($vo["type"] == 1): ?><a href="<?php echo U('sales/view','id='.$vo['bill_id']);?>"><?php else: ?><a href="<?php echo U('purchase/view','id='.$vo['bill_id']);?>"><?php endif; echo ($vo["bill"]); ?></a></td>
							<td><?php if($vo["type"] == 1): ?>销售退货<?php else: ?>采购<?php endif; ?></td>
							<?php if(C('ismobile') != 1): ?><td><?php echo ($vo["price"]); ?></td>
								<!--td><?php echo (date("Y-m-d",$vo['pay_time'])); ?></td-->
								<td>
									<?php if($vo['status'] == 2): ?>已付款<?php elseif($vo['status'] == 1): ?>部分已付<font color="red">(<?php echo L('ARREARAGE',array($vo['un_payable']));?>)</font><?php else: ?>未付款<?php endif; ?>
								</td>
							...
							<td>
								...
								<?php else: ?>
									<?php if($vo["status"] != '2'): ?><!--a href="<?php echo U('finance/edit','t=payables&id='.$vo['payables_id']);?>"><?php echo L('EDIT');?></a-->
										<a class="payablesorder" rel="<?php echo ($vo['payables_id']); ?>" href="javascript:void(0);"><?php echo L('ADD PAYMENTORDER');?></a><?php endif; endif; ?>
							</td>
...
<div class="hide" id="dialog-payablesorder" title="<?php echo L('ADD RECEIVINGORDER');?>">loading...</div>
<script type="text/javascript">
<?php if(C('ismobile') == 1): ?>width=$('.container').width() * 0.9;<?php else: ?>width=800;<?php endif; ?>
	...
	$(function(){
		...
		$(".payablesorder").click(function(){
			var id = $(this).attr('rel');
			$('#dialog-payablesorder').dialog('open');
			$('#dialog-payablesorder').load('<?php echo U("finance/adddialog","t=paymentorder&id=");?>'+id);
		});
	});
</script>
