...
<div class="container">
	...
	<a href="<?php echo U('task/index','by=sub');?>" <?php if($_GET['by']== 'sub'): ?>class="active"<?php endif; ?>><?php echo L('SUBORDINATE_TASK');?></a> | 
						...
						<thead>
							<tr>
								...
								<?php elseif(C('ismobile') != 1): ?>
									...
									<th><?php echo L('END_DATE');?></th><?php endif; ?>
						...
						<tbody>
							<?php if(is_array($task_list)): $i = 0; $__LIST__ = $task_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
									...
									<td>
										...
										<?php if($vo['isclose'] == 0 and $_GET['by']!= 'deleted' and $vo['creator']['role_id'] == $_SESSION['role_id']): ?><a href="<?php echo U('task/close','id='.$vo['task_id']);?>"><?php echo L('CLOSE');?> &nbsp;</a><?php elseif($_GET['by']!= 'deleted' and $vo['isclose'] == 1): ?><a href="<?php echo U('task/open','id='.$vo['task_id']);?>"><?php echo L('OPEN');?> &nbsp;</a><?php endif; ?>
									</td>
