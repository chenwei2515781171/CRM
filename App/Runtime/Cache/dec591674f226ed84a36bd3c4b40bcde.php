					...
					<form class="form-inline" id="searchForm" onsubmit="return checkSearchForm();" action="" method="get">
						<ul class="nav pull-left" style="margin-top:20px">
							...
							<li class="pull-left">
								...
								<input type="hidden" name="zyact" id="zyact" value="search"/>
								<input name="m" value="finance" type="hidden">
								<input name="a" value="ziyong" type="hidden">
		...
		<div  id="an_chart" style="margin-left:10px;">			
			<div id="report_content">
				<table class="table table-hover">
					<thead>
						<tr>
							<th style="text-align:center;height:30px;line-height:30px;border:1px #F0F0F0 solid;" colspan="8">自用统计</th>
							<th><a href="javascript:void(0);" id="ziyongexcel" class="link"> <i class="icon-download"> </i> 导出表格</a></th>
						</tr>
						<tr>
							<th>客户</th>
							<th>销售单号</th>
...
/*$("#ziyongexcel").click(function(){
	if(confirm("你确定要导出自用数据吗？")){
		$("#zyact").val('zyexcel');	
		$("#searchForm").submit();
	}
})
...
$("#ziyongexcel").click(function(){
	if(confirm("你确定要导出自用数据吗？")){
		...
		var url = "<?php echo U('finance/ziyong');?>&customer="+customer+"&role="+role+"&start_time="+start_time+"&end_time="+end_time+"&act=zyexcel";
