...
* @return	array	返回字符串
*/
function getUserNameByRoleId($roleid){
	if(!$roleid) return '';
	$user_id = M('role')->where("role_id=$roleid")->getField('user_id');
	$name = M('user')->where("user_id=$user_id")->getField('name');
	return $name;
}
/**
* 将字符串转换为数组
	...
	if ($position_list) {
		...
		foreach($position_list AS $value) {
			...
			foreach($user_list as $v){
				$user_str .= '<a style="color: #000000;" href="'.U('user/view','id='.$v['user_id']).'" target="_blank">'.$v['user_name'].'、</a>';
			}
		...
		if ($v['field'] == 'customer_id') {
            ...
            switch ($v['form_type']) {
                ...
                case 'address':
                    ...
                    </script><select id="state" name="'.$v['field'].'[\'state\']" class="input-medium"></select> 
                        <select id="city" name="'.$v['field'].'[\'city\']" class="input-medium"></select>
                        <input type="text" name="'.$v['field'].'[\'street\']" placeholder="'.L('THE STREET INFORMATION').'" class="input-large" value="'.$street.'" id="address">';
                    break;
                case 'p_box':
                        $str = '';
...
function getTableInfo($table_name){
	$sql = 'SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE table_name LIKE "'.$table_name.'"';
	$result = M('')->query($sql);
	return $result;
}

//发送短信
function send_sms($m,$c){
	$data = array(
		'action'=>'sendOnce',
		'ac'=>'1001@501192700001',
		'authkey'=>'F8384FE43A9BF8BEB42833111A2A7916',
		'cgid'=>'3911',
		'm'=>$m,
		'c'=>$c,
		//'c'=>iconv('gbk','utf-8',$c),
	...
	$row = parse_url('http://smsapi.c123.cn/OpenPlatform/OpenApi');
	$host = $row['host'];
