...
class ProductAction extends Action {
	...
	public function _initialize(){
		$action = array(
			...
			'allow'=>array('listdialog','changecontent','adddialog','editdialog', 'mdelete','allproductdialog','validate','check','mutildialog','d_changecontent')
		);
...
//			$this->categoryList = $categoryList;
	...
	public function excelExport($productList=false){
		...
		$objProps->setTitle("lxcrm Product");    
		$objProps->setSubject("lxcrm Product Data");    
		$objProps->setDescription("lxcrm Product Data");    
		$objProps->setKeywords("lxcrm Product");    
        ...
        header("Content-Disposition:attachment;filename=lxcrm_product_".date('Y-m-d',mktime()).".xls");
	...
	public function excelImportDownload(){
		...
		$objProps->setSubject("lxcrm Product Data");    
		$objProps->setDescription("lxcrm Product Data");    
		$objProps->setKeywords("lxcrm Product Data");    
		$objProps->setCategory("lxcrm");
		$objPHPExcel->setActiveSheetIndex(0);     
		$objActSheet = $objPHPExcel->getActiveSheet(); 
		...
		$objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
		header("Content-Type: application/vnd.ms-excel;");
        header("Content-Disposition:attachment;filename=lxcrm_product.xls");
        header("Pragma:no-cache");
        header("Expires:0");
        $objWriter->save('php://output'); 
    }
	...
	public function listDialog(){
		...
		$product_list = $product->order('product_id asc')->page($p,C('PAGE_SIZE'))->select();
	...
	public function d_changecontent(){
		if($this->isAjax()){
			$product = D('ProductView');
			$where = array();
			if ($_REQUEST["field"]) {
			...
			$count = $product->where($where)->count();// 查询满足要求的总记录数
			$list = $product->where($where)->Page($p.',10')->select();
			foreach($list as $k=>$v){
				$stock_count = M('stock')->where('product_id = %d', $v['product_id'])->sum('amounts');
				$list[$k]['stock_count'] = empty($stock_count) ? $list[$k]['stock_count'] = 0 : $list[$k]['stock_count'] = $stock_count;
			}
    ...
    public function mutildialog(){
		...
		$tree->icon = array('&nbsp;&nbsp;&nbsp;│ ','&nbsp;&nbsp;&nbsp;├─ ','&nbsp;&nbsp;&nbsp;└─ ');
		$tree->nbsp = '&nbsp;&nbsp;&nbsp;';
		$category_data = $m_product_category->select();
		$str="<span rel=\$category_id class='file ta'>\$name</span>";
		$str2="<span rel=\$category_id  class='folder ta'>\$name</span>";
		$tree->init($category_data);
		$gg = $tree->get_treeview(0,'category_tree',$str,$str2);
		...
		//页码等参数赋值 这里对应系统js参数
		$data['data']['total'] = ceil($count/C('PAGE_SIZE'));//总页码
		$data['data']['p'] = $p;//总页码
		$data['data']['count'] = $count;//总shu
	...
	public function changeContent(){
		if($this->isAjax()){
			...
			if ($_REQUEST["cid"]) {
				$where['category_id'] = array('eq',$_REQUEST["cid"]);
			}
