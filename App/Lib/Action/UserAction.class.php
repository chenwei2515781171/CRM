...
class UserAction extends Action {
		...
		if (session('?name')){
			...
			if((!isset($_POST['name']) || $_POST['name'] =='')||(!isset($_POST['password']) || $_POST['password'] =='')){
				...
				if ($user['password'] == md5(md5(trim($_POST['password'])) . $user['salt'])) {				
					...
					alert('error', L('INCORRECT_USER_NAME_OR_PASSWORD'),U('user/login'));
				}
			}			
			$this->alert = parseAlert();
			$this->display();
		}else{
			$subcompany = M('subcompany')->select();
	...
	public function logout() {
		...
		$this->success(L('LOGIN_OUT_SUCCESS'));
		echo "<script>window.location.href='http://crm.hdyujiu.com/index.php?m=user&a=login'</script>";
	}
		...
		if ($this->isPost()){
			...
			if($_POST['radio_type'] == 'email'){
				...
				if (!isset($_POST['name']) || $_POST['name'] == '') {
					...
					alert('error', L('PERCENT_FORMAT_ERROR'), $_SERVER['HTTP_REFERER']);
				}*/
				$m_user->status = 1;
				//为用户设置默认导航（根据系统菜单设置中的位置）
				$m_navigation = M('navigation');
