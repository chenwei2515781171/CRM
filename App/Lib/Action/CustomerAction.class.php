...
class CustomerAction extends Action {
	...
	public function _initialize(){
		$action = array(
			'permission'=>array(),
			'allow'=>array('getcustomerlist','analytics', 'validate','check', 'fenpei', 'revert','changecontent','customerlock','check_customer_limit','excelImportDownload','search','listDialog','sx')
		);
	...
	*/
	...
	public function changeContent(){
		if($this->isAjax()){
			...
			//制单人获取全部人客户
			$create_sale_user = M('config') -> where('name="create_sale_user"')->getField('value');
			if(in_array(session("user_id"),explode(',',$create_sale_user))){
				$underling_ids = getAllRoleId();
			}else{
	...
	public function add(){
		if(C('ismobile')) $this->redirect('customer/m_add');
		if($this->isPost()){
	...
	public function m_add(){
		if($this->isPost()){		
			...
			//客户来源
			$origin = M('fields')->where("model='customer' and field='origin'")->getField('setting');
			$origin = string2array($origin);
			$origin_list = $origin['data'];
			$this->assign('origin_list',$origin_list);

			//客户行业
			$industry = M('fields')->where("model='customer' and field='industry'")->getField('setting');
			$industry = string2array($industry);
			$industry_list = $industry['data'];
			$this->assign('industry_list',$industry_list);

			//公司性质
			$ownership = M('fields')->where("model='customer' and field='ownership'")->getField('setting');
			$ownership = string2array($ownership);
			$ownership_list = $ownership['data'];
			$this->assign('ownership_list',$ownership_list);

			//客户评分
			$rating = M('fields')->where("model='customer' and field='rating'")->getField('setting');
			$rating = string2array($rating);
			$rating_list = $rating['data'];
			$this->assign('rating_list',$rating_list);

            $alert = parseAlert();
			$this->assign('active_customer','true');
            $this->alert = $alert;
            $this->display();
		}
	}
	...
	public function edit(){
		if(C('ismobile')) $this->redirect('customer/m_edit');
	...
	public function m_edit(){
		...
		if(!check_permission($id, 'customer')) $this->error(L('HAVE NOT PRIVILEGES'));
		$customer = D('CustomerView')->where('customer.customer_id = %d',$id)->find();
		$sales_list = M('sales')->where("customer_id=$id")->field('id,sn_code,sales_price')->select();
		...
		if($this->isPost()){
			...
			$data['ownership'] = $this->_post('ownership');
			$data['rating'] = $this->_post('rating');
			$data['address'] = implode(chr(10),$this->_post('address'));
			$a = M('customer')->where("customer_id=$id")->save($data);
			$info['name'] = $this->_post('con_name');
			$info['post'] = $this->_post('con_post');
			$info['telephone'] = $this->_post('con_telephone');
			$b = M('contacts')->where("contacts_id=$customer[contacts_id]")->save($info);
			if($a !== false && $b !== false){
				alert('success', L('EDIT_CLIENTS_SUCCESS'), U('customer/index'));
			}else{
				alert('error', L('CUSTOMER_EDITING_FAILED!'),$_SERVER['HTTP_REFERER']);
			...
			$customer['contacts'] = M('contacts')->where('contacts_id = %d', $customer['contacts_id'])->field('name,post,telephone')->find();
			$address = explode(chr(10),$customer['address']);
	...
	public function index(){
		if(C('ismobile')) $this->redirect('customer/m_index');
		$d_v_customer = D('CustomerView');
        $by = isset($_GET['by']) ? trim($_GET['by']) : '';
		$below_ids = getSubRoleId(false);
		$all_ids = getSubRoleId();
	...
	public function m_index(){
		if($this->isAjax()){
			...
			$where['owner_role_id'] = $where_role_id;
			$where['is_deleted'] = '0';

			$p = !$_REQUEST['p']||$_REQUEST['p']<=0 ? 1 : intval($_REQUEST['p']);
			$list = D('CustomerView')->where($where)->order('create_time desc')->field('customer_id,name,contacts_id')->page($p.',10')->select();
			foreach ($list as $k => $v) {
				$list[$k]["telephone"] = M('contacts')->where("contacts_id=$v[contacts_id]")->getField('telephone');
			}
			$data['list'] = $list;
			$data['nextp'] = $p+1;
			$this->ajaxReturn($data,"",1);
			...
			$list = D('CustomerView')->where($where)->order('create_time desc')->field('customer_id,name,contacts_id')->limit('10')->select();
			foreach ($list as $k => $v) {
				$list[$k]["telephone"] = M('contacts')->where("contacts_id=$v[contacts_id]")->getField('telephone');
			}	
			$count = D('CustomerView')->where($where)->order('create_time desc')->field('customer_id,name,contacts_id')->count();
			$this->customerlist = $list;
	...
	public function caresEdit(){
		...
		if($care_id && !check_permission($care_id, 'customer')) $this->error(L('HAVE NOT PRIVILEGES'));
	...
	public function sx(){
		...
		if($keyword){
			$data = M("customer")->where("NAME LIKE '%".$keyword."%'")->select();
			if($data){
				foreach($data as $v){
					$str .= "<option value=\"".$v['customer_id']."\" >".$v['name']."</option>";
				}
				echo $str;
			}
		}
