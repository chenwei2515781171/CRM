...
class LogAction extends Action{
	public function _initialize(){
		$action = array(
			...
			'allow'=>array('add', 'view', 'delete', 'edit', 'index', 'mylog_view', 'mylog_edit', 'log_delete', 'mylog_add', 'anly','tasklog')
		);
	...
	public function mylog_view(){
		if($_GET['id']){
			...
			$pre = M('Log')->where('role_id = %d and category_id = %d and create_date < %d', $log['role_id'],$log['category_id'], $log['create_date'])->order('create_date desc')->limit(1)->find();
			if($pre) $this->pre_href = U('log/mylog_view', 'id='.$pre['log_id']);
			$next = M('Log')->where('role_id = %d and category_id = %d and create_date > %d ',$log['role_id'],$log['category_id'],  $log['create_date'])->limit(1)->find();
