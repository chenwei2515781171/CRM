...
class SettingAction extends Action{
	...
	public function defaultinfo(){
		if($this->isGet()){
			...
			$this->assign('bonus_pool', $bonus_pool);
			...
			$this->assign('check_send', explode(',',$check_send));
			$outorder_send = M('config') -> where('name="outorder_send"')->getField('value');
			$this->assign('outorder_send', explode(',',$outorder_send));
			$create_sale_user = M('config') -> where('name="create_sale_user"')->getField('value');
			$this->assign('create_sale_user', explode(',',$create_sale_user));

			//获取填写手机号码的人员
			$user_list = M('user')->where(array('status'=>1,'telephone'<>''))->field('name,user_id')->select();
			...
			//处理短信发送人员
			if(!empty($_POST['check_send'])){
				$check_send_str = implode(',',$_POST['check_send']);
				$m_config->where('name = "check_send"')->setField('value', $check_send_str);
			}
			if(!empty($_POST['outorder_send'])){
				$outorder_send_str = implode(',',$_POST['outorder_send']);
				$m_config->where('name = "outorder_send"')->setField('value', $outorder_send_str);
			}
			if(!empty($_POST['create_sale_user'])){
				$create_sale_user_str = implode(',',$_POST['create_sale_user']);
				$m_config->where('name = "create_sale_user"')->setField('value', $create_sale_user_str);
			}
			...
			$bonus_pool = M('config') -> where('name="bonus_pool"') -> setField('value',$_POST['bonus_pool']);
			...
			if($result_defaultinfo || $contract_custom  || $leads_outdays || $result_customer_outdays || $result_customer_limit_condition || $result_customer_limit_counts || $bonus_pool){
	...
	public function subCompany(){
		...
		$subcompany_list = M('subcompany')->order('id desc')->Page($p.','.C('PAGE_SIZE'))->select();

		$count = M('subcompany')->count();
		$Page = new Page($count,C('PAGE_SIZE'));// 实例化分页类 传入总记录数和每页显示的记录数
		$show = $Page->show();// 分页显示输出

		$this->assign('subcompany_list',$subcompany_list);
	...
	public function addsubcompany(){
		if($this->isPost()){
			if(!$_POST['name']) alert('error',L('NAME_CAN_NOT_EMPTY'),$_SERVER['HTTP_REFERER']);
			$data['name'] = $this->_post('name','strip_tags,htmlspecialchars');
			$data['description'] = $this->_post('description','strip_tags,htmlspecialchars','');
			$data['url'] = $this->_post('url','strip_tags,htmlspecialchars','');
			$data['create_time'] = time();
			if(M('subcompany')->add($data)){
	...
	public function editsubcompany(){
		if($this->isPost() && isset($_POST['id'])){
			...
			$data['description'] = $this->_post('description','strip_tags,htmlspecialchars','');
			$data['url'] = $this->_post('url','strip_tags,htmlspecialchars','');
			$r = M('subcompany')->where("id=$id")->save($data);
			if($r){
			...
			$info = M('subcompany')->where("id=$id")->find();
	...
	public function deletecompany(){
		if($id = intval($_GET['id'])){
			M('subcompany')->where("id=$id")->delete();
			alert('success',L('DELETE SUCCESSFULLY'),$_SERVER['HTTP_REFERER']);
		}else{
			alert('error',L('PARAMETER_ERROR'),$_SERVER['HTTP_REFERER']);
		}
	}
