...
class ContractAction extends Action {
		...
		if($this->isPost()){
			...
			if($data['type']==1){
				...
				if(empty($data['supplier_id'])){
					alert('error','请选择供应商',$_SERVER['HTTP_REFERER']);
				}
			}
			...
			if($contractId = $contract->add($data)){
				...
				if($_POST['refer_url']){
					alert('error', L('FAILED_TO_CREATE_THE_CONTRACT'), $_POST['refer_url']);
				}else{
	...
	public function edit(){
		...
		if($contract_info['type']==2){
			$contract_info['contacts_name'] = M('supplier_contact')->where("supplier_id=$contract_info[supplier_id]")->getField('name');
		}
	...
	public function view(){
		...
		if($info['type']==2){
			$info['contacts_name'] = M('supplier_contact')->where("supplier_id=$info[supplier_id]")->getField('name');
		}
