...
class ContactsAction extends Action {
	...
	public function _initialize(){
		$action = array(
			'permission'=>array(),
			'allow'=>array('checklistdialog','getContactsList', 'revert', 'mdelete','radiolistdialog','changedialog','add_dialog','qrcode')
		);
	...
	public function edit(){
        ...
        if( $contacts_id && !check_permission($contacts_id, 'contacts')) $this->error(L('HAVE NOT PRIVILEGES'));
		if ($_GET['id']) {
	...
	public function view(){
		...
		//if(!check_permission($contacts_id, 'contacts')) $this->error(L('HAVE NOT PRIVILEGES'));
		if (0 == $contacts_id) {
	...
	public function index(){
		...
		if(trim($_GET['act']) == 'excel'){
			if(vali_permission('contacts', 'export')){
				...
				$contactsList = $m_contacts->where($where)->order($order)->select();		
